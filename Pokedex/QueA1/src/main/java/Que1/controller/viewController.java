...
public class viewController implements Initializable {
    ...
    {
        final pokedexData pdd = new pokedexData();
        pokemonSearch.textProperty().addListener(new ChangeListener<String>()
        {
            ...
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) 
            {
                ...
                if (pdd.CheckValid(pokemonSearch.getText().toLowerCase()))
                {
                    ...
                    String[][] returnPokemon = pdd.getPokemonData(pokemonSearch.getText().toLowerCase());
                    String outputString = "";           
                    for (int i = 0; i < returnPokemon.length; i++) 
                    {
                        for (int j = 0; j < returnPokemon[i].length; j++) 
                        {   
                            if(returnPokemon[i][j] != null)
                            {
                                outputString += String.format("%s ", returnPokemon[i][j]);
                            }
